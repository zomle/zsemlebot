	...
	public partial class TwitchService
	{
		private void HandleCommand(string? sourceMessageId, string channel, MessageSource sender, string command, string? parameters)
		{
			switch (command)
			{
				...
				case Constants.Command_Streak:
					HandleStreakCommand(sourceMessageId, channel, sender, parameters);
					break;

				case Constants.Command_Today:
					HandleTodayCommand(sourceMessageId, channel, sender, parameters);
					break;
			}
		}

		private void HandleStreakCommand(string? sourceMessageId, string channel, MessageSource sender, string? parameters)
		{
		...
		private void HandleStreakCommandForHotaUsers(string? sourceMessageId, string channel, string queriedUser, IReadOnlyList<HotaUser> hotaUsers)
		{
			...
			{
				SendChatMessage(sourceMessageId, channel, MessageTemplates.UserNotFound(queriedUser));
				return;
			}
		...
		private void HandleTodayCommand(string? sourceMessageId, string channel, MessageSource sender, string? parameters)
		{
			if (string.IsNullOrEmpty(parameters))
			{
				//check for current channel
